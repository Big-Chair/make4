...
- **Match transport** (`OnlineMatchTransport`) — the only Room seam the Match sees: `role`, `status`, `send`, `subscribe`. Its identity is stable for one Room generation. The Match never receives Room creation, joining, Player Token, invitation, failure, or Supabase state.
