...
- **Interrupted Room** — a Ready Room whose local channel is lost, whose peer's Presence disappears, or whose Match reported a revision gap. Match input and its timer pause at once, and the interruption starts **one** 20-second reconnect-and-resynchronization deadline (it is not reset by flickers within the interruption). The Room state is `interrupted` with a `reason` (`peer-left | channel-lost | revision-gap`) and `resynchronizing` (both sessions live again, snapshot handshake under way).
- **Participant session** — the Presence `clientId` a Room Module generates once for its lifetime. Both sessions are latched when the Room becomes ready; only those sessions may reconnect. Any other identity claiming a Role is refused (`resync-failed`).
- **Same-session reconnect** — when both latched sessions are live again the transport becomes `resynchronizing` and the Match runs the Match Snapshot handshake from both ends (the guest requests, the host pushes); neither peer resumes before the acknowledgement.
- **Reload refusal** — the active Room code is remembered for the page session; a Room Module that finds it on mount starts `failed` instead of recovering. Full-page reload recovery is not supported.
